...
2. Read `supply-code/tmp/NEXT_TICKET.json`.
   - **`authoring=stencil`** (proved clone: 6.5 billing relegation, or contempt of a 6.5 writ dismissed): run `python tools/scj_stencil.py --write` then step 4. **Do not** read the judgment or write JSON by hand. If write fails: `python tools/prepare_next_scj.py --demote` then follow short/full — **do not** re-prepare the same PDF as stencil. Listing-only / 4.4 / 6.5-*refusals* / court-grants (SCJ-411 Lok Adalat compliance) are **not** stencil.
