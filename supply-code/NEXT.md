...
3. Write **only** `supply-code/summaries/json/<case_id>.json` (lean schema). Include `page_count` (source PDF pages, from the ticket/fp) and `significance` (`significant` | `ordinary` | `procedural`; `normal` = ordinary). Never invent citations/holdings. `cited_by` is a string; `lead_authorities` is `[{name, docid}, …]`; provision keys are `CODE::clause`. Thin orders still recorded, with a `flag`. Unresolved points → `not_decided[]`.
